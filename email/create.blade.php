		...
		if( $("#user_type").val() == "Student" && $('select[name=section_id]').val() !=""){	
			...
			$.ajax({
				...
				beforeSend: function(){
					...
					jQuery.each( json, function( i, val ) {
					   $('select[name=student_id]').append("<option value='"+val['email']+"'>"+val['roll']+" - "+val['first_name']+"</option>");
					});
					...
					jQuery.each( json, function( i, val ) {
					   ...
					   .append('<div class="col-md-12">'+
									'<label class="c-container">'+
									   '<input type="checkbox" value="'+val['email']+'" name="students[]" checked="true">'+val['roll']+" - "+val['first_name']+

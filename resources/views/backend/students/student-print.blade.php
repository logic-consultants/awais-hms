...
@yield('content')
<div style="background:white;padding:80px;margin-top:20;px" id="student_print_div">
<table>
  <tr>
    <td colspan="2">
    ...
    <td colspan="2">
		<img style="width:150px;height:150px;border: 2px solid grey;padding:10px;" src="{{ asset('uploads/images/'.$student->image) }}">
    </td>
  </tr>
</table> 
	<div class="row">
						...
						<td colspan="4">I am Agreed on Monthly Rent</td>
							<table class="table">
								...
								<tbody id="invoice_assign">
									@if(count($data['assign_fees']) > 0)
									@foreach($data['assign_fees'] as $item)
									<tr>
										<td width="40%">{!! get_fee_selectbox('selection', $item->fee_id) !!}</td>
										...
										<td><input type="text" class="form-control" value="{{ $item->amount - $item->discount }}" readonly></td>
									</tr>
									@endforeach
									@endif
								</tbody>
								...
								<tr>
									...
									<td><input type="text" class="form-control" id="total_assign" name="total" value="{{ $data['assign_fees']->sum('amount') - $data['assign_fees']->sum('discount') }}" readOnly="true"></td>
								</tr>
								</tfoot>
							</table>
					</tr>
